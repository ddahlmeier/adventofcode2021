...
if __name__ == "__main__":
    ...
    while len(stack)>0:
        ...
        for next_scanner, next_rotation, next_translation in alignments[scanner]:
            if next_scanner not in positions.keys():
                stack.append((next_scanner, position + rotation.dot(next_translation), rotation.dot(next_rotation)))

    max_distance = 0
    for scan1 in range(len(scans)-1):
        for scan2 in range(scan1+1, len(scans)):
            dist = manhatten_distance(positions[scan1], positions[scan2])
            print("distance between scanners", scan1, scan2, "=", dist)
